...
import dev.dev48v.shipping.orchestration.OrchestrationProperties;
import dev.dev48v.shipping.orchestration.SagaCommand;
import dev.dev48v.shipping.orchestration.SagaReply;
...
// Day 29 — the Kafka wiring for shipping-service's ORCHESTRATION command handler, added beside the Day-28
// choreography configs. shipping-service CONSUMES SagaCommand from saga-commands and PRODUCES SagaReply onto
// saga-replies. Bean names are distinct from the choreography configs and the template is typed to SagaReply,
// so there is no collision with the existing ShippingEventPublisher's Object-typed template. The handler's
// autoStartup is gated by shipping.orchestration.enabled (default false), so these containers stay dormant
// unless orchestration is switched on.
@Configuration
@EnableKafka
